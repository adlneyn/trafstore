...
if (window.tailwind) {
    tailwind.config = {
        theme: {
            extend: { colors: { primary: '#1e3a8a', secondary: '#0f172a', accent: '#eff6ff' } }
        }
    }
}
...
// Eksekusi Render Data ke HTML setelah halaman dimuat
document.addEventListener("DOMContentLoaded", () => {
    
    // Fungsi bantuan untuk me-render kotak menu
    const renderCard = (title, price, onclickAction, iconClass) => `
        <div class="bg-white p-3 rounded-lg border border-gray-200 menu-card flex justify-between items-center">
            <div>
                <h3 class="font-bold ${price ? 'text-xs' : 'text-sm'} text-secondary">${title}</h3>
                ${price ? `<p class="text-xs text-primary font-semibold">RP.${price}</p>` : ''}
            </div>
            <button onclick="${onclickAction}" class="bg-accent text-primary w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center hover:bg-primary hover:text-white transition ml-2">
                <i class="${iconClass}"></i>
            </button>
        </div>
    `;
    ...
    document.getElementById('grid-lpm').innerHTML = lpmData.map(num => renderCard(num, num, `orderViaWA('${num} LPM', 'Jaseb LPM')`, 'fa-solid fa-arrow-right')).join('');
    ...
    document.getElementById('grid-lpfp').innerHTML = Object.entries(lpfpPrices).map(([num, price]) => renderCard(num, price, `orderViaWA('${num} LPFP', 'Jaseb LPFP')`, 'fa-solid fa-arrow-right')).join('');
    ...
    document.getElementById('grid-editing').innerHTML = editingData.map(item => renderCard(item, null, `orderViaWA('${item}', 'Editing')`, 'fa-solid fa-cart-plus')).join('');
    ...
    document.getElementById('grid-wording').innerHTML = wordingData.map(item => renderCard(item, null, `orderViaWA('${item}', 'Wording')`, 'fa-solid fa-cart-plus')).join('');
    ...
    document.getElementById('grid-am').innerHTML = amData.map(i => renderCard(i.n, i.p, `orderViaWA('${i.n}', 'App Premium')`, 'fa-solid fa-arrow-right text-xs')).join('');
    ...
    document.getElementById('grid-spot').innerHTML = spotData.map(i => renderCard(i.n, i.p, `orderViaWA('${i.n}', 'App Premium')`, 'fa-solid fa-arrow-right text-xs')).join('');
    ...
    document.getElementById('grid-editapp').innerHTML = editAppData.map(i => renderCard(i.n, i.p, `orderViaWA('${i.n}', 'App Premium')`, 'fa-solid fa-arrow-right text-xs')).join('');
    ...
    document.getElementById('grid-stream').innerHTML = streamData.map(i => renderCard(i.n, i.p, `orderViaWA('${i.n}', 'App Premium')`, 'fa-solid fa-arrow-right text-xs')).join('');
    ...
    document.getElementById('grid-ai').innerHTML = aiData.map(i => renderCard(i.n, i.p, `orderViaWA('${i.n}', 'App Premium')`, 'fa-solid fa-arrow-right text-xs')).join('');
    ...
    document.getElementById('grid-aovlol').innerHTML = aovLol.map(i => renderCard(i.n, i.p, `orderViaWA('${i.n}', 'Top Up Game')`, 'fa-solid fa-arrow-right text-xs')).join('');
    ...
    document.getElementById('grid-ff').innerHTML = ffData.map(i => renderCard(i.n, i.p, `orderViaWA('${i.n}', 'Top Up Game')`, 'fa-solid fa-arrow-right text-xs')).join('');
    ...
    document.getElementById('grid-hoyo').innerHTML = hoyoData.map(i => renderCard(i.n, i.p, `orderViaWA('${i.n}', 'Top Up Game')`, 'fa-solid fa-arrow-right text-xs')).join('');
    ...
    document.getElementById('grid-othergames').innerHTML = otherGames.map(i => renderCard(i.n, i.p, `orderViaWA('${i.n}', 'Top Up Game')`, 'fa-solid fa-arrow-right text-xs')).join('');

    // 7. LAINNYA
    const lainnyaData = ["NOKOS", "SEWA BOT", "KEBSOS WA", "KEBSOS TELEGRAM", "KEBSOS INSTAGRAM", "KEBSOS TIKTOK", "KEBSOS FACEBOOK"];
    document.getElementById('grid-lainnya').innerHTML = lainnyaData.map(item => renderCard(item, null, `orderViaWA('${item}', 'Lainnya')`, 'fa-solid fa-arrow-right')).join('');
});